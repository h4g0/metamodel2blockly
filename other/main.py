...
from visualizemetamodel import visualize_ecore_model
...
travis_instances_validation="travisinstancesvalidation"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run various tests and visualizations.")
    parser.add_argument("--visualize", action="store_true", help="Visualize the Ecore model.")
    parser.add_argument("--run_all_ga", action="store_true", help="Run all GitHub Actions tests.")
    parser.add_argument("--run_all_travis", action="store_true", help="Run all Travis tests.")
    parser.add_argument("--run_test_ga", type=str, help="Run a specific GitHub Actions test file.")
    parser.add_argument("--run_test_travis", type=str, help="Run a specific Travis test file.")

    args = parser.parse_args()

    if args.visualize:
        visualize_ecore_model(metamodel_location)

    if args.run_all_ga:
        run_all_tests_ga()

    if args.run_all_travis:
        run_all_tests_travis()

    if args.run_test_ga:
        run_test_file_ga(args.run_test_ga)

    if args.run_test_travis:
        run_test_file_travis(args.run_test_travis)

    if args.copy_files:
        copy_random_files(travis_instances, travis_instances_validation, 10)

    if args.count_ga:
        print(count_files(ga_instances))

    if args.count_ga_validation:
        print(count_files(ga_validation_instances))

    if args.check_keys_travis:
        get_check_keys_travis()
main()