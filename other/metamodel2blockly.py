...
def test_metamodel2blockly(metamodel_path, output_format='web'):
    ...
    if output_format == 'web':
        ...
        # Copy entire visualci project
        visualci_src = os.path.join('interface', 'visualci')
        if os.path.exists(visualci_src):
            # Copy all files recursively - compatible with Python <3.8
            for item in os.listdir(visualci_src):
                src_path = os.path.join(visualci_src, item)
